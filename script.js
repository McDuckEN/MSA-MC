let eventsInitialized = false;
const OPENAI_API_KEY = "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";


let requestInProgress = false;

// جلوگیری از ارسال‌های پشت‌سرهم
let lastRequestTime = 0;
const MIN_DELAY = 1500

function initializeEventListeners() 
{
    if (eventsInitialized) return;  // جلوگیری از دوبار اجرا شدن
    eventsInitialized = true;

    // ... تمام event listenerهای قبلی ...
}



class AIChatAssistant 
{
    
    constructor() 
    {
        this.conversationHistory = [];
        this.isAPIEnabled = true;
        
        this.chatGPTApiKey = "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"; // API Key خودتان را قرار دهید
        this.chatGPTEndpoint = "https://api.openai.com/v1/responses";
        this.isChatGPTAvailable = false;
        
        this.initializeEventListeners();
        this.checkChatGPTAvailability();
        
        console.log('🔧 دستیار دانشگاه منتظری با ChatGPT راه‌اندازی شد');
    }

    async checkChatGPTAvailability() 
    {
        try 
        {
            const response = await fetch('https://api.openai.com/v1/models', {
                headers: {
                    'Authorization': `Bearer ${this.chatGPTApiKey}`
                }
            });
            
            if (response.ok) 
            {
                this.isChatGPTAvailable = true;
                this.updateStatus('✅ متصل به ChatGPT');
                console.log('✅ اتصال به ChatGPT برقرار است');
            } 
            else 
            {
                throw new Error('اتصال برقرار نشد');
            }
        } 
        catch (error)
         {
            console.warn('⚠️ ChatGPT در دسترس نیست:', error);
            this.isChatGPTAvailable = false;
            this.updateStatus('🌐 استفاده از مدل داخلی');
        }
        
        //this.showWelcomeMessage();
    }

    updateStatus(message) 
    {
        const statusElement = document.getElementById('api-status');
        if (statusElement) 
        {
            statusElement.innerHTML = `<span>${message}</span>`;
        }
    }

    showWelcomeMessage() 
    {
        setTimeout(() => {
            if (this.isChatGPTAvailable) 
            {
                this.displayMessage(
                    "🎓 **دستیار هوشمند دانشگاه منتظری**\n\n" +
                    "✨ **قدرت گرفته از ChatGPT 3.5 Turbo** ✨\n\n" +
                    "من می‌توانم به سوالات شما درباره:\n" +
                    "• 📚 برنامه کلاس‌ها و امتحانات\n" +
                    "• 🎓 ثبت‌نام و انتخاب واحد\n" +
                    "• 👨‍🏫 اساتید و اطلاعات دروس\n" +
                    "• 📖 کتابخانه و منابع آموزشی\n" +
                    "• 🏠 خوابگاه و امکانات\n" +
                    "• 💰 شهریه و وام‌های دانشجویی\n" +
                    "• ⚖️ قوانین انضباطی\n\n" +
                    "سوال خود را بپرسید...",
                    'bot'
                );
            } 
            else 
            {
                this.displayMessage(
                    "🎓 **دستیار دانشگاه منتظری**\n\n" +
                    "⚠️ **ChatGPT در دسترس نیست**\n\n" +
                    "در حال حاضر از هوش مصنوعی داخلی استفاده می‌شود.\n\n" +
                    "📞 **راه‌های ارتباطی دانشگاه:**\n" +
                    "• تلفن: ۰۲۱-۶۶۱۵۰۰۰۰\n" +
                    "• سایت: www.montazeri-univ.ac.ir\n" +
                    "• سامانه: sama.montazeri.ac.ir\n\n" +
                    "سوال خود را بپرسید...",
                    'bot'
                );
            }
        }, 500);
    }

    initializeEventListeners() 
    {
        const sendBtn = document.getElementById('send-btn');
        const userInput = document.getElementById('user-input');
        const quickBtns = document.querySelectorAll('.quick-btn');

        if (sendBtn) 
        {
            sendBtn.addEventListener('click', () => this.handleUserInput());
        }
        
        if (userInput) 
        {
            userInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') 
                {
                    this.handleUserInput();
                }
            });

            setTimeout(() => userInput.focus(), 1500);
        }

        if (quickBtns) 
        {
            quickBtns.forEach(btn => {
                btn.addEventListener('click', () => {
                    const question = btn.getAttribute('data-question');
                    this.askQuestion(question);
                });
            });
        }

        const apiSettings = document.querySelector('.api-settings');
        if (apiSettings) 
        {
            apiSettings.style.display = 'none';
        }
    }

    async handleUserInput() 
    {
        const userInput = document.getElementById('user-input');
        if (!userInput) return;
        
        const question = userInput.value.trim();
        
        if (question) 
        {
            await this.askQuestion(question);
            userInput.value = '';
        }
    }

async askQuestion(question) {
    this.displayMessage(question, 'user');
    this.showTypingIndicator();
    
    try {
        let answer;

        if (this.isChatGPTAvailable && this.isAPIEnabled) {
            console.log('🔄 درخواست به ChatGPT ارسال می‌شود...');
            answer = await this.getChatGPTResponse(question);
        } 
        else if (this.isBotpressAvailable) {
            console.log('🔄 درخواست به Botpress ارسال می‌شود...');
            answer = await this.getBotpressResponse(question);
        } 
        else {
            await new Promise(resolve => setTimeout(resolve, 800));
            answer = this.getEnhancedResponse(question);
        }

        this.hideTypingIndicator();
        this.displayMessage(answer, 'bot');

        // افزودن به تاریخچه مکالمه
        this.conversationHistory.push({
            question,
            answer,
            timestamp: new Date().toISOString(),
            source: this.isChatGPTAvailable ? 'chatgpt' : (this.isBotpressAvailable ? 'botpress' : 'local')
        });
        
    } catch (error) {
        this.hideTypingIndicator();
        console.error('Error in askQuestion:', error);
        const fallbackAnswer = this.getEnhancedResponse(question);
        this.displayMessage(fallbackAnswer, 'bot');
    }
}

// تابع جدید برای ارسال درخواست به Botpress
async getBotpressResponse(question) {
    const response = await fetch('YOUR_BOTPRESS_API_URL', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ message: question }),
    });
    const data = await response.json();
    return data.reply || 'متاسفانه پاسخ مناسبی پیدا نشد.';
}


    async getChatGPTResponse(userMessage) {
        // جلوگیری از ارسال سریع پشت سر هم
        if (requestInProgress) return "لطفاً چند لحظه صبر کنید...";
        const now = Date.now();
        if (now - lastRequestTime < MIN_DELAY) return "در حال پردازش هستم، یک لحظه صبر کن...";
        
        requestInProgress = true;
        lastRequestTime = now;
    
        try {
            const response = await fetch("https://api.openai.com/v1/responses", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${this.chatGPTApiKey}`
                },
                body: JSON.stringify({
                    model: "gpt-4.1-mini",
                    input: [
                        { role: "system", content: "You are a helpful assistant." },
                        { role: "user", content: userMessage }
                    ]
                })
            });
            
            const data = await response.json();
            
            // پاسخ اصلی
            if (!response.ok || data.error) {
                console.warn("❌ خطای OpenAI:", data);
                return this.getEnhancedResponse(userMessage); // fallback داخلی
            }
            
            // پاسخ از API جدید
            return data.output_text || this.getEnhancedResponse(userMessage);
            
            
        } catch (err) {
            console.error("❌ خطای شبکه:", err);
            // fallback به پاسخ داخلی
            return this.getEnhancedResponse(userMessage);
        } finally {
            requestInProgress = false;
        }
    }
    

    getEnhancedResponse(question) 
    {
        // پایگاه دانش پیشرفته برای دانشگاه منتظری
        const enhancedKnowledge = 
        {
            "کلاس": [
                "📚 **برنامه کلاس‌های ترم پاییز  دانشگاه منتظری:**\n\n" +
                "🏫 **دوره‌های آموزشی:**\n" +
                "• شروع ترم: ۵ مهر   \n" +
                "• پایان ترم: ۲۸ دی   \n" +
                "• ساعات کاری: ۸:۰۰-۱۲:۰۰ و ۱۴:۰۰-۱۸:۰۰\n\n" +
                "🎯 **نکات مهم:**\n" +
                "• برنامه کامل در سامانه سما قابل مشاهده است\n" +
                "• برای تغییر برنامه با مدیر گروه هماهنگ کنید\n" +
                "• غیبت بیش از ۴ جلسه منجر به محرومیت می‌شود",

                "⏰ **زمان‌بندی کلاس‌ها دانشگاه منتظری**\n\n" +
                "📍 **محل برگزاری:**\n" +
                "• کلاس‌های نظری: ساختمان اصلی\n" +
                "• کلاس‌های عملی: آزمایشگاه دانشکده فنی\n" +
                "• کارگاه‌ها: ساختمان کارگاه‌های تخصصی\n\n" +
                "📱 **دسترسی به برنامه:**\n" +
                "• سامانه سما → برنامه هفتگی\n" +
                "• انتخاب ترم جاری\n" +
                "• مشاهده برنامه رشته خود"
            ],

            "برنامه کلاس": [
                "🗓️ **برنامه هفتگی دانشگاه منتظری**\n\n" +
                "📅 **روزهای آموزشی:**\n" +
                "• شنبه تا چهارشنبه: روزهای آموزشی\n" +
                "• پنجشنبه: کلاس‌های جبرانی\n" +
                "• جمعه: تعطیل\n" +
                "• برنامه هر رشته در سامانه سما موجود است\n\n" +
                "📱 **دسترسی:** سما → برنامه هفتگی → انتخاب ترم",

                "📅 **برنامه ترمی دانشگاه**\n\n" +
                "🗓️ **تاریخ‌های مهم:**\n" +
                "• میان ترم: هفته ۸ آموزشی\n" +
                "• پایان ترم: ۲۰ دی تا ۱۰ بهمن\n" +
                "• تعطیلات: در تقویم آموزشی مشخص شده\n" +
                "• تغییرات: از طریق سامانه اعلام می‌شود"
            ],

            "امتحان": [
                "📝 **برنامه امتحانات ترم پاییز   **\n\n" +
                "🗓️ **تاریخ‌های مهم:**\n" +
                "• امتحانات میان ترم: هفته ۸ آموزشی\n" +
                "• امتحانات پایان ترم: ۲۰ دی تا ۱۰ بهمن\n" +
                "• اعلام برنامه: ۲ هفته قبل از امتحانات\n\n" +
                "🏛️ **محل برگزاری:**\n" +
                "• سالن‌های A, B, C ساختمان مرکزی\n" +
                "• زمان مراجعه: ۳۰ دقیقه قبل از شروع",

                "🎯 **قوانین امتحانات دانشگاه منتظری**\n\n" +
                "📋 **مقررات:**\n" +
                "• کارت دانشجویی الزامی است\n" +
                "• وسایل شخصی مجاز نیست\n" +
                "• تاخیر حداکثر ۱۵ دقیقه مجاز\n" +
                "• غیبت بیش از حد = محرومیت\n\n" +
                "💡 **توصیه:**\n" +
                "• منابع درسی را از سامانه LMS دریافت کنید\n" +
                "• در کلاس‌های جبرانی شرکت کنید"
            ],

            "پایان ترم": [
                "🏁 **امتحانات پایان ترم   **\n\n" +
                "🗓️ **برنامه زمانی:**\n" +
                "• شروع: ۲۰ دی ماه   \n" +
                "• پایان: ۱۰ بهمن   \n" +
                "• برنامه نهایی: ۱۵ دی اعلام می‌شود\n" +
                "• سالن‌ها: A, B, C ساختمان مرکزی\n\n" +
                "📊 **نتایج:** ۱۵ روز پس از آخرین امتحان",

                "📚 **آمادگی برای پایان ترم**\n\n" +
                "🎯 **راهنمایی‌ها:**\n" +
                "• منابع: در سامانه LMS موجود است\n" +
                "• کلاس‌های جبرانی: هفته ۱۴ برگزار می‌شود\n" +
                "• مشاوره تحصیلی: واحد مشاوره دانشگاه\n" +
                "• کتابخانه: تا ۲۲ شب فعال است"
            ],

            "میان ترم": [
                "📋 **امتحانات میان ترم**\n\n" +
                "⏰ **زمان‌بندی:**\n" +
                "• زمان: هفته ۸ آموزشی\n" +
                "• محتوا: نیمی از مطالب تدریس شده\n" +
                "• ضریب: ۳۰٪ نمره پایانی\n" +
                "• برنامه: توسط اساتید اعلام می‌شود\n\n" +
                "🎓 **تاثیر:** نمره میان ترم در معدل موثر است",

                "⏳ **برنامه میان ترم   **\n\n" +
                "🗓️ **جزئیات:**\n" +
                "• تاریخ: ۲۰ آذر تا ۲۵ آذر\n" +
                "• محل: کلاس‌های معمول\n" +
                "• مدت: ۱ ساعت\n" +
                "• نمره دهی: تا ۱۰ روز پس از امتحان"
            ],

            "ثبت نام": [
                "🎓 **ثبت نام ترم پاییز    دانشگاه منتظری**\n\n" +
                "⏰ **زمان‌بندی:**\n" +
                "• شروع: ۱۵ شهریور   \n" +
                "• پایان: ۱۰ مهر   \n" +
                "• مهلت تغییر: تا هفته دوم\n\n" +
                "💻 **روش ثبت نام:**\n" +
                "• ورود به سامانه سما\n" +
                "• انتخاب واحد\n" +
                "• پرداخت شهریه\n" +
                "• تایید نهایی",

                "📄 **مدارک مورد نیاز برای ثبت نام**\n\n" +
                "✅ **الزامی:**\n" +
                "• تصویر کارت دانشجویی\n" +
                "• فیش واریزی شهریه\n" +
                "• عکس ۳*۴ جدید\n\n" +
                "🔧 **پشتیبانی فنی:**\n" +
                "• واحد فناوری اطلاعات\n" +
                "• داخلی: ۱۲۳\n" +
                "• ساعات کاری: ۸-۱۶"
            ],

            "انتخاب واحد": [
                "📋 **شرایط انتخاب واحد دانشگاه منتظری**\n\n" +
                "📊 **حدود واحد:**\n" +
                "• حداکثر واحد: ۲۰ واحد\n" +
                "• دانشجویان ممتاز: ۲۴ واحد (معدل بالای ۱۷)\n" +
                "• حداقل واحد: ۱۲ واحد\n" +
                "• مهلت تغییر: تا پایان هفته دوم\n\n" +
                "🎯 **ملاحظات:**\n" +
                "• دروس پیش‌نیاز رعایت شود\n" +
                "• تداخل زمانی بررسی شود\n" +
                "• تایید مدیر گروه برای واحد اضافی",

                "🔧 **راهنمای انتخاب واحد**\n\n" +
                "📝 **نکات مهم:**\n" +
                "• پیش‌نیازها: در سایت دانشگاه چک کنید\n" +
                "• ظرفیت‌ها: به صورت لحظه‌ای در سما\n" +
                "• تغییرات: تا هفته دوم امکان‌پذیر\n" +
                "• مشکلات: با مدیر گروه هماهنگ شود\n\n" +
                "💡 **توصیه:** با راهنمای آموزشی مشورت کنید"
            ],

            "کتابخانه": [
                "📖 **کتابخانه مرکزی دانشگاه منتظری**\n\n" +
                "🕒 **ساعات کاری:**\n" +
                "• روزهای عادی: ۸:۰۰-۲۰:۰۰\n" +
                "• پنجشنبه‌ها: ۸:۰۰-۱۳:۰۰\n" +
                "• جمعه‌ها: تعطیل\n\n" +
                "📚 **امکانات:**\n" +
                "• ۵۰,۰۰۰ جلد کتاب\n" +
                "• مقالات بین‌المللی\n" +
                "• پایگاه‌های علمی\n" +
                "• سالن مطالعه",

                "🔍 **خدمات کتابخانه دانشگاه**\n\n" +
                "📖 **قوانین امانت:**\n" +
                "• حداکثر ۵ کتاب\n" +
                "• مدت امانت: ۲ هفته\n" +
                "• امکان تجدید: ۲ بار\n" +
                "• عضویت اجباری\n\n" +
                "💻 **سرویس‌های آنلاین:**\n" +
                "• جستجوی پیشرفته\n" +
                "• رزرو کتاب\n" +
                "• تمدید آنلاین"
            ],

            "خوابگاه": [
                "🏠 **خوابگاه‌های دانشگاه منتظری**\n\n" +
                "🎯 **شرایط اسکان:**\n" +
                "• اولویت با دانشجویان شهرستانی\n" +
                "• ظرفیت: ۵۰۰ دانشجو\n" +
                "• هزینه: ترمی ۲,۵۰۰,۰۰۰ تومان\n\n" +
                "🏢 **امکانات:**\n" +
                "• اینترنت پرسرعت\n" +
                "• سالن مطالعه\n" +
                "• سالن ورزش\n" +
                "• آشپزخانه مشترک",

                "📝 **ثبت نام خوابگاه   **\n\n" +
                "🗓️ **زمان‌بندی:**\n" +
                "• شروع ثبت نام: ۱۰ شهریور\n" +
                "• اعلام نتایج: ۲۰ شهریور\n" +
                "• تحویل خوابگاه: ۱ مهر\n\n" +
                "📄 **مدارک مورد نیاز:**\n" +
                "• تصویر شناسنامه\n" +
                "• کارت دانشجویی\n" +
                "• عکس ۳*۴\n" +
                "• معرفی نامه"
            ],

            "شهریه": [
                "💵 **شهریه ترم پاییز    دانشگاه منتظری**\n\n" +
                "💰 **مقادیر شهریه:**\n" +
                "• شهریه ثابت: ۳,۰۰۰,۰۰۰ تومان\n" +
                "• شهریه متغیر: واحد‌ای ۱۵۰,۰۰۰ تومان\n" +
                "• مثال (۱۶ واحد): ۵,۴۰۰,۰۰۰ تومان\n\n" +
                "⏰ **مهلت پرداخت:**\n" +
                "• تا پایان هفته سوم\n" +
                "• جریمه تاخیر: ۱۵٪",

                "🏦 **روش‌های پرداخت شهریه**\n\n" +
                "💳 **گزینه‌های پرداخت:**\n" +
                "• درگاه بانکی سامانه سما\n" +
                "• پرداخت در محل به حسابدار\n" +
                "• طرح اقساط (با هماهنگی)\n\n" +
                "📞 **اطلاعات بیشتر:**\n" +
                "• اداره امور مالی\n" +
                "• داخلی: ۱۲۱\n" +
                "• ساعات: ۸-۱۵"
            ],

            "استاد": [
                "👨‍🏫 **اطلاعات اساتید دانشگاه منتظری**\n\n" +
                "🌐 **دسترسی به اطلاعات:**\n" +
                "• سایت دانشگاه → اعضای هیئت علمی\n" +
                "• سامانه LMS\n" +
                "• دفتر هر دانشکده\n\n" +
                "📞 **روش‌های ارتباط:**\n" +
                "• ایمیل دانشگاهی\n" +
                "• ساعات ملاقات مشخص\n" +
                "• سامانه پیام‌رسان LMS",

                "💼 **ساعات ملاقات اساتید**\n\n" +
                "📅 **برنامه هفتگی:**\n" +
                "• در دفتر هر دانشکده نصب شده\n" +
                "• در سایت دانشگاه موجود است\n" +
                "• معمولاً بین ساعات ۱۰-۱۲\n\n" +
                "💡 **توصیه:**\n" +
                "• برای ملاقات قبلاً هماهنگ کنید\n" +
                "• خارج از ساعات اداری ایمیل بزنید"
            ],

            "معدل": [
                "📊 **سیستم نمره‌دهی دانشگاه منتظری**\n\n" +
                "🎯 **حدود نمره:**\n" +
                "• حداقل قبولی: ۱۰\n" +
                "• مشروطی: معدل زیر ۱۲\n" +
                "• اخطار تحصیلی: معدل زیر ۱۰\n\n" +
                "🔄 **حذف ترم:**\n" +
                "• یک بار در طول تحصیل\n" +
                "• تا هفته چهارم امکان‌پذیر",

                "📈 **راه‌های ارتقاء معدل**\n\n" +
                "🎓 **استراتژی‌ها:**\n" +
                "• اخذ مجدد دروس با نمره پایین\n" +
                "• انتخاب واحدهای اختیاری\n" +
                "• شرکت در کلاس‌های جبرانی\n\n" +
                "🤝 **پشتیبانی:**\n" +
                "• مشاوره با راهنمای آموزشی\n" +
                "• خدمات واحد مشاوره دانشگاه"
            ],

            "انضباطی": [
                "⚖️ **قوانین انضباطی دانشگاه منتظری**\n\n" +
                "📋 **مقررات:**\n" +
                "• غیبت مجاز: ۴ جلسه در هر درس\n" +
                "• محرومیت از امتحان: غیبت بیش از حد\n" +
                "• شئونات اسلامی: رعایت کامل الزامی است\n" +
                "• تخلفات: در شورای انضباطی بررسی می‌شود\n\n" +
                "📄 **آیین‌نامه:** در سایت دانشگاه موجود است",

                "🎓 **آیین‌نامه انضباطی دانشجویان**\n\n" +
                "⚠️ **موارد مهم:**\n" +
                "• حضور منظم در کلاس‌ها\n" +
                "• رعایت رفتار مناسب\n" +
                "• پوشش اسلامی\n" +
                "• حفظ اموال دانشگاه\n\n" +
                "⏰ **اعتراض:** تا ۷۲ ساعت پس از ابلاغ\n" +
                "🤝 **مشاوره:** اداره آموزش"
            ],

            "وام": [
                "🏦 **وام‌های دانشجویی دانشگاه منتظری**\n\n" +
                "💰 **انواع وام:**\n" +
                "• وام تحصیلی: ۱۵ میلیون تومان\n" +
                "• وام فوری: ۵ میلیون تومان\n" +
                "• وام شهریه: تا سقف شهریه\n\n" +
                "✅ **شرایط:**\n" +
                "• معدل بالای ۱۴\n" +
                "• عدم اقساط معوقه\n" +
                "• ضامن معتبر",

                "📑 **مراحل دریافت وام تحصیلی**\n\n" +
                "👣 **مراحل اداری:**\n" +
                "۱. مراجعه به امور دانشجویی\n" +
                "۲. تکمیل فرم درخواست\n" +
                "۳. تایید مدیر گروه\n" +
                "۴. تحویل مدارک\n\n" +
                "⏰ **مدت پرداخت:**\n" +
                "• حداکثر ۱۰ روز کاری"
            ],

            "تغییر رشته": [
                "🔄 **تغییر رشته در دانشگاه منتظری**\n\n" +
                "⏰ **زمان:**\n" +
                "• فقط ابتدای سال تحصیلی\n" +
                "• شرط معدل: بالای ۱۵\n" +
                "• مراحل:\n" +
                "  ۱. دریافت فرم از اداره آموزش\n" +
                "  ۲. تایید مدیر گروه مبدا و مقصد\n" +
                "  ۳. ارائه به شورای آموزشی\n" +
                "  ۴. صدور مجوز\n\n" +
                "📅 **مهلت:** تا پایان هفته دوم مهر",

                "📋 **شرایط تغییر رشته**\n\n" +
                "📊 **شرایط لازم:**\n" +
                "• معدل کل: حداقل ۱۵\n" +
                "• تعداد واحد گذرانده: حداکثر ۴۰ واحد\n" +
                "• ظرفیت رشته مقصد: موجود باشد\n" +
                "• پیش‌نیازها: بررسی شود\n\n" +
                "💡 **توصیه:** با مشاور آموزشی مشورت کنید"
            ]
        };

        const lowerQuestion = question.toLowerCase();
        
        // جستجوی هوشمند
        let bestResponse = null;
        let bestScore = 0;

        for (const [keyword, responses] of Object.entries(enhancedKnowledge)) 
        {
            const score = this.calculateSimilarity(lowerQuestion, keyword);
            if (score > bestScore)
             {
                bestScore = score;
                bestResponse = responses[Math.floor(Math.random() * responses.length)];
            }
        }

        if (bestScore > 0.3 && bestResponse) 
        {
            return bestResponse;
        }

        // پاسخ عمومی
        return this.getGeneralResponse(question);
    }

    calculateSimilarity(question, keyword) 
    {
        const questionWords = question.split(' ');
        const keywordWords = keyword.split(' ');
        
        let matches = 0;
        questionWords.forEach(qWord => 
        {
            keywordWords.forEach(kWord => 
            {
                if (qWord.includes(kWord) || kWord.includes(qWord)) 
                {
                    matches++;
                }
            });
        });
        
        return matches / Math.max(questionWords.length, keywordWords.length);
    }

    getGeneralResponse(question) 
    {
        const responses = [
            `🤔 **سوال: "${question}"**\n\n` +
            `📞 **برای اطلاعات دقیق:**\n` +
            `• تماس با دانشگاه: ۰۲۱-۶۶۱۵۰۰۰۰\n` +
            `• سایت: www.montazeri-univ.ac.ir\n` +
            `⏰ **ساعات کاری:** ۷:۳۰-۱۸:۰۰`,

            `💡 **در مورد سوال شما**\n\n` +
            `"${question}"\n\n` +
            `🎯 **راه‌های دریافت اطلاعات:**\n` +
            `✅ سامانه آموزشی دانشگاه\n` +
            `✅ اداره آموزش\n` +
            `✅ مدیر گروه\n` +
            `✅ واحد مشاوره\n\n` +
            `📱 **پشتیبانی:**19-38781016 `,

            `🔍 **پاسخ به سوال شما**\n\n` +
            `برای "${question}"\n\n` +
            `🌐 **منابع اطلاعاتی:**\n` +
            `• سایت دانشگاه\n` +
            `• سامانه سما\n` +
            `• دفترچه راهنما\n` +
            `• اطلاعیه‌ها\n\n` +
            `📞 **تماس:**19-38781016 `
        ];

        return responses[Math.floor(Math.random() * responses.length)];
    }

    getBasicResponse(question) 
    {
        const knowledgeBase = 
        {
            "کلاس": "برنامه کلاس‌ها در سامانه آموزشی دانشگاه منتظری قابل مشاهده است.",
            "امتحان": "برنامه امتحانات دو هفته قبل از شروع در سامانه اعلام می‌شود.",
            "ثبت نام": "ثبت نام از طریق سامانه سما انجام می‌شود.",
            "استاد": "لیست اساتید در سایت دانشگاه موجود است.",
            "کتابخانه": "کتابخانه از ۸ صبح تا ۸ شب فعال است.",
            "خوابگاه": "خوابگاه‌های دانشگاه برای دانشجویان شهرستانی در نظر گرفته شده است.",
            "شهریه": "اطلاعات شهریه در سامانه مالی دانشگاه قابل مشاهده است.",
            "وام": "برای دریافت وام به اداره امور دانشجویی مراجعه کنید."
        };
        
        const lowerQuestion = question.toLowerCase();
        for (const [key, answer] of Object.entries(knowledgeBase)) 
        {
            if (lowerQuestion.includes(key)) 
            {
                return answer;
            }
        }
        
        return "برای اطلاعات بیشتر با اداره آموزش دانشگاه منتظری تماس بگیرید: 19-38781016 ";
    }

    displayMessage(message, sender) 
    {
        const chatMessages = document.getElementById('chat-messages');
        if (!chatMessages) return;
        
        const messageDiv = document.createElement('div');
        messageDiv.className = `message ${sender}-message`;
        messageDiv.innerHTML = `
            <div class="message-content">
                ${message.replace(/\n/g, '<br>')}
                <div class="message-time">${this.getCurrentTime()}</div>
            </div>
        `;
        
        chatMessages.appendChild(messageDiv);
        chatMessages.scrollTop = chatMessages.scrollHeight;
    }

    showTypingIndicator() 
    {
        const typingIndicator = document.getElementById('typing-indicator');
        if (typingIndicator) 
        {
            typingIndicator.style.display = 'block';
            const chatMessages = document.getElementById('chat-messages');
            if (chatMessages) 
            {
                chatMessages.scrollTop = chatMessages.scrollHeight;
            }
        }
    }

    hideTypingIndicator() 
    {
        const typingIndicator = document.getElementById('typing-indicator');
        if (typingIndicator) 
        {
            typingIndicator.style.display = 'none';
        }
    }

    getCurrentTime() 
    {
        const now = new Date();
        return now.toLocaleTimeString('fa-IR', { 
            hour: '2-digit', 
            minute: '2-digit',
            hour12: false 
        });
    }

    // قابلیت‌های مدیریتی
    clearChat() 
    {
        const chatMessages = document.getElementById('chat-messages');
        if (chatMessages) 
        {
            chatMessages.innerHTML = '';
            this.conversationHistory = [];
            this.displayMessage(
                "💬 **مکالمه جدید آغاز شد**\n\n" +
                "سلام! من دستیار هوشمند دانشگاه منتظری هستم.\n" +
                "چگونه می‌توانم کمک‌تان کنم؟ 🌟",
                'bot'
            );
        }
    }

    exportConversation() 
    {
        if (this.conversationHistory.length > 0) 
        {
            const data = 
            {
                university: "دانشگاه منتظری",
                timestamp: new Date().toISOString(),
                conversations: this.conversationHistory
            };
            
            const dataStr = JSON.stringify(data, null, 2);
            const blob = new Blob([dataStr], {type: 'application/json'});
            const url = URL.createObjectURL(blob);
            
            const a = document.createElement('a');
            a.href = url;
            a.download = `university-chat-${new Date().toLocaleDateString('fa-IR')}.json`;
            a.click();
            
            URL.revokeObjectURL(url);
            
            this.displayMessage("✅ مکالمه با موفقیت ذخیره شد", 'bot');
        } 
        else 
        {
            this.displayMessage("💡 هیچ مکالمه‌ای برای ذخیره کردن وجود ندارد", 'bot');
        }
    }
}

// راه‌اندازی خودکار
document.addEventListener('DOMContentLoaded', function() 
{
    window.universityAssistant = new AIChatAssistant();
    
    // اضافه کردن دکمه‌های مدیریتی اگر وجود دارند
    const clearBtn = document.getElementById('clear-chat');
    if (clearBtn) 
    {
        clearBtn.addEventListener('click', () => window.universityAssistant.clearChat());
    }
    
    const exportBtn = document.getElementById('export-chat');
    if (exportBtn) 
    {
        exportBtn.addEventListener('click', () => window.universityAssistant.exportConversation());
    }
    
    console.log('🚀 دستیار دانشگاه منتظری با ChatGPT راه‌اندازی شد');
});

// تابع برای تنظیم API Key جدید
function setOpenAIAPIKey(newKey) 
{
    if (window.universityAssistant) 
    {
        window.universityAssistant.chatGPTApiKey = newKey;
        console.log('API Key به روز شد');
        window.universityAssistant.checkChatGPTAvailability();
    }
}

// تابع درباره ما
function showAbout() 
{
    const aboutText = `
🎓 **دستیار هوشمند دانشگاه منتظری**

🎓 **طراح و برنامه نویس:** امیر محمد نصیری
🏢 **شرکت:** مک داک 
📱 *5.3.1*نسخه:** 

✨ **ویژگی‌های سیستم:**
✅ پاسخ‌های هوشمند و طبیعی
✅ اطلاعات دقیق دانشگاه منتظری
✅ رابط کاربری پیشرفته

📞 **راه‌های ارتباطی دانشگاه:**
• تلفن: 19-38781016
• سایت: www.montazeri-univ.ac.ir`;
    
    alert(aboutText);
    return false;
}

// ===== Botpress Webchat Dynamic Embed =====
// (function () {
//   // ساخت iframe
//   const iframe = document.createElement("iframe");

//   iframe.src = "https://cdn.botpress.cloud/webchat/v3.5/shareable.html?configUrl=https://files.bpcontent.cloud/2025/12/12/22/20251212221222-IPZDCY3A.json";
//   iframe.style.position = "fixed";
//   iframe.style.bottom = "20px";
//   iframe.style.right = "20px";
//   iframe.style.width = "380px";
//   iframe.style.height = "520px";
//   iframe.style.border = "none";
//   iframe.style.zIndex = "100000"; /* بالاتر از همه */
//   iframe.style.boxShadow = "0 8px 24px rgba(0,0,0,0.2)";
//   iframe.style.borderRadius = "8px";

//   // اگر لازم باشه یک دکمه باز/بسته کن
//   const toggleBtn = document.createElement("button");
//   toggleBtn.textContent = "💬 Chat";
//   toggleBtn.style.position = "fixed";
//   toggleBtn.style.bottom = "20px";
//   toggleBtn.style.right = "20px";
//   toggleBtn.style.zIndex = "100001";
//   toggleBtn.style.padding = "12px 18px";
//   toggleBtn.style.background = "#007bff";
//   toggleBtn.style.color = "#fff";
//   toggleBtn.style.fontSize = "16px";
//   toggleBtn.style.border = "none";
//   toggleBtn.style.borderRadius = "50px";
//   toggleBtn.style.cursor = "pointer";
//   toggleBtn.style.boxShadow = "0 4px 12px rgba(0,0,0,0.15)";

//   let open = false;

//   toggleBtn.onclick = () => {
//     open = !open;
//     if (open) {
//       document.body.appendChild(iframe);
//       toggleBtn.textContent = "❌ Close";
//     } else {
//       iframe.remove();
//       toggleBtn.textContent = "💬 Chat";
//     }
//   };

//   document.body.appendChild(toggleBtn);
// })();
